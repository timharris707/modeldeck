...
| Provider marks | Official logos — Claude starburst in Anthropic clay `#D97757`, OpenAI knot in white. Sourced from hiverunner `ProviderLogo`. |
...
| Refresh | Manual + optional background auto-refresh (interval setting, default 5 min) with optional "pause while a session is active". Footer shows "Updated N min ago" + manual Refresh. |
